class Concept < ActiveRecord::Base
  ...
  def self.find_all_exchange_programs
    Concept.find(:all, :order => "title", :joins => "join wiki_references on wiki_references.concept_id = concepts.id", :conditions => ["wiki_references.link_type = 'C' AND wiki_references.referenced_name = ?", "Mainų programos"])
  end

  def self.find_all_activity_areas
    Concept.find(:all, :order => "title", :joins => "join wiki_references on wiki_references.concept_id = concepts.id", :conditions => ["wiki_references.link_type = 'C' AND wiki_references.referenced_name = ?", "Veiklos sritys"])
  end
